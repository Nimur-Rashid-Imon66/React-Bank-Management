        ...
        if (!flag[0]) {
            ...
            setUserData(UserData.map((user) => {
                if (user.accountNo == accountNo) {
                    if (user.accountBalance[currencyCode]) {
                        user.accountBalance[currencyCode] += Number(amount);
                    }
                    else {
                        user.accountBalance[currencyCode] = Number(amount);
                    }
                    const transactionId = uniqid();
                    user.creditHistory[transactionId] = {
                        amount: amount,
    ...
    return (
        <div>
            ...
            <div
                ...
                <h1 className='text-blue-500 text-center border-b-[4px] pb-2 border-blue-500  text-4xl font-semibold mb-6'> Add Income </h1>
